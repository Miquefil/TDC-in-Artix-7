...
EXACT_PATH = 'C:/Users/mique/Desktop/TDC-in-Artix-7/TESTS/Mediciones finales'
...
for direccion in os.listdir():      ##devuelve lista de archivos y folders
    if(len(direccion.split('.'))!=2):   #si no es un archivo(si es archivo split separa nombre de archivo y extension)
        os.chdir(EXACT_PATH)
        os.chdir(f'./{direccion}/')
        lineas = []
        for archivo in os.listdir():
            nombre_archivo = archivo.split('_')
            if((nombre_archivo[0] != 'resultados')and (nombre_archivo[0] != 'resultadosCSV')):    ## que no appendee desde mismo archivo donde se escriben los resultados
...
for nombre_file in os.listdir():
    if(len(nombre_file.split('.'))!=2):       ##si no es un archivo
        print(nombre_file)
        os.chdir(EXACT_PATH)
